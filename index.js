...
(function() {
  var Asset, AssetGraph, UglifyJS, appcacheParse, appcacheRender, crypto, css, fs, htmlparser, join, os, path, requireJS, shell, _;
  ...
  AssetGraph = (function() {
    ...
    AssetGraph.prototype.generateAppCache = function(destination, manifest) {
      var a, currentManifest, elem, input, now, out, p, tokens, _i, _len, _ref, _ref1;
      if (manifest + '' === 'true') {
        manifest = 'manifest.appcache';
        console.log('generating ', manifest);
        tokens = this._generateAppCacheTokens();
      } else {
        currentManifest = join(this.root, manifest);
        if (!fs.existsSync(currentManifest)) {
          ...
          tokens = appcacheParse(input, {
            tokenize: true
          });
          now = new Date();
          tokens.splice(1, 0, {
            type: 'comment',
            value: now.toString("dd/M/yy h:mm tt")
          });
          tokens.push({
            type: 'newline'
          });
      ...
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        elem = _ref1[_i];
        if (elem.type === 'tag' && elem.name === 'html') {
          elem.attribs.manifest = manifest;
        }
      }
      this.nodes['index.html'].writeToDisc(destination);
      out = appcacheRender(tokens, {
        tokenized: true
      });
      manifest = join(destination, manifest);
      console.log('writing appcache manifest to ', manifest);
      return fs.writeFileSync(manifest, out);
    };

    AssetGraph.prototype._generateAppCacheTokens = function() {
      var a, now, p, tokens, _ref;
      tokens = [
        {
